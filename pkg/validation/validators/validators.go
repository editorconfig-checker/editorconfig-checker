...
func Charset(rawFileContent []byte, charsetWanted string, charsetFound string, config config.Config) error {
	// lowercase everything as that's how they're listed in the ec spec.
	charsetWanted = strings.ToLower(charsetWanted)
	if !encoding.IsSupportedCharset(charsetWanted) {
	...
	bomEncoding := encoding.DetectByBOM(rawFileContent)
	if !encoding.CharsetsEqual(charsetFound, charsetWanted, bomEncoding) {
