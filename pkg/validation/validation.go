...
func ValidateFile(filePath string, config config.Config) []error.ValidationError {
	...
	fileInformation = files.FileInformation{Content: fileContent, FilePath: filePath, Editorconfig: definition}
	validationError = ValidateCharset(fileInformation, config, charset, rawFileContent)
	if validationError.Message != nil {
		validationErrors = append(validationErrors, validationError)
	}
...
func ValidateCharset(fileInformation files.FileInformation, config config.Config, charset string, rawFileContent []byte) error.ValidationError {
